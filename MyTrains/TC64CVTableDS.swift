...
//  MyTrains
//
//  Created by Paul Willmott on 29/12/2022.
//

import Foundation
import Cocoa

public class TC64CVTableDS : NSObject, NSTableViewDataSource, NSTableViewDelegate {
    ...
    case ColumnIdentifiers.EnabledColumn:
      if let cell = tableView.makeView(withIdentifier:
        ...
        if let button = cell.subviews[0] as? NSButton {
          button.tag = row
          button.state = item.nextIsEnabled ? .on : .off
        }
        return cell
      }
