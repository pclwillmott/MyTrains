  ...
  public func readCVs(selectedCVs:Set<Int>) {
    ...
    if let network = self.network, let interface = network.interface as? InterfaceLocoNet {
      ...
      if !selectedSwitches.isEmpty {
        ...
        interface.readCV(progMode: .operations, cv: currentCV, address: UInt16(boardId))
  ...
  public func writeCVs(selectedCVs:Set<Int>) {
    ...
    if let network = self.network, let interface = network.interface as? InterfaceLocoNet {
      ...
      if !selectedSwitches.isEmpty {
        ...
        interface.writeCV(progMode: .operations, cv: currentCV, address: Int(boardId), value: UInt16(cvs[currentCV - 1].nextCVValue))
...
//              currentCV = selectedSwitches.first!
              ...
              interface.writeCV(progMode: .operations, cv: currentCV, address: boardId, value: UInt16(cvs[currentCV - 1].nextCVValue))
